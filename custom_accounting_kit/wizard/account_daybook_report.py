...
class AccountDayBookReport(models.TransientModel):
    _name = "custom.account.daybook.report"
    ...
    account_ids = fields.Many2many('account.account', 'custom_account_account_daybook_report', 'report_line_id',
                                   'account_id', 'Accounts')
    ...
    def check_report(self):
        ...
        return self.env.ref(
            'custom_accounting_kit.action_report_day_book').report_action(self,
                                                                     data=data)
