...
class FollowupPrint(models.TransientModel):
    _name = 'custom.followup.print'
    _description = 'Print Follow-up & Send Mail to Customers'

    def _get_followup(self):
        if self.env.context.get('active_model',
                                'ir.ui.menu') == 'custom.followup.followup':
            return self.env.context.get('active_id', False)
        company_id = self.env.user.company_id.id
        followp_id = self.env['custom.followup.followup'].search(
    ...
    followup_id = fields.Many2one('custom.followup.followup', 'Follow-Up',
                                  required=True, readonly=True,
                                  default=_get_followup)
    partner_ids = fields.Many2many('custom.followup.stat.by.partner',
                                   'custom_partner_stat_rel', 'osv_memory_id',
    ...
    def process_partners(self, partner_ids, data):
        ...
        nbprints = 0
        resulttext = " "
        for partner in self.env['custom.followup.stat.by.partner'].browse(
                partner_ids):
    ...
    def clear_manual_actions(self, partner_list):
        partner_list_ids = [partner.partner_id.id for partner in self.env[
            'custom.followup.stat.by.partner'].browse(partner_list)]
    ...
    def do_process(self):
        ...
        resource_id = self.env.ref(
            'custom_accounting_kit.view_custom_account_followup_sending_results')
        ...
        return {
            ...
            'context': context,
            'view_mode': 'list,form',
            'res_model': 'custom.followup.sending.results',
