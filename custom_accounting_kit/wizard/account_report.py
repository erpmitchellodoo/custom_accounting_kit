...
class AccountingReport(models.TransientModel):
    ...
    def _get_account_report(self):
        ...
        if self.env.context.get('active_id'):
            ...
            reports = self.env['custom.account.financial.report'].search([('name', 'ilike', menu)])
        return reports and reports[0] or False

    enable_filter = fields.Boolean(string='Enable Comparison')
    account_report_id = fields.Many2one('custom.account.financial.report', string='Account Reports',
    ...
    def _print_report(self, data):
        ...
        return self.env.ref('custom_accounting_kit.action_report_financial').report_action(self, data=data, config=False)
