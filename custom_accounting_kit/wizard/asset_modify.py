...
class AssetModify(models.TransientModel):
    ...
    def _get_asset_method_time(self):
        if self.env.context.get('active_id'):
            asset = self.env['custom.account.asset.asset'].browse(self.env.context.get('active_id'))
    ...
    def default_get(self, fields):
        ...
        asset = self.env['custom.account.asset.asset'].browse(asset_id)
        if 'name' in fields:
        ...
        if self.env.context.get('active_id'):
            active_asset = self.env['custom.account.asset.asset'].browse(self.env.context.get('active_id'))
    ...
    def modify(self):
        ...
        tracked_fields = self.env['custom.account.asset.asset'].fields_get(['method_number', 'method_period', 'method_end'])
