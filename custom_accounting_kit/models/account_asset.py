...
class AccountAssetCategory(models.Model):
    _name = 'custom.account.asset.category'
...
class AccountAssetAsset(models.Model):
    _name = 'custom.account.asset.asset'
    ...
    category_id = fields.Many2one(
        'custom.account.asset.category', string='Category',
    ...
    depreciation_line_ids = fields.One2many(
        'custom.account.asset.depreciation.line', 'asset_id', string='Depreciation Lines'
    )
    ...
    def compute_generated_entries(self, date, asset_type=None):
        ...
        ungrouped_assets = self.env['custom.account.asset.asset'].search(type_domain + [('state', '=', 'open'), ('category_id.group_entries', '=', False)])
        created_move_ids += ungrouped_assets._compute_entries(date, group_entries=False)

        for grouped_category in self.env['custom.account.asset.category'].search(type_domain + [('group_entries', '=', True)]):
            assets = self.env['custom.account.asset.asset'].search([('state', '=', 'open'), ('category_id', '=', grouped_category.id)])
    ...
    def validate(self):
        ...
        ref_tracked_fields = self.env['custom.account.asset.asset'].fields_get(fields)
        for asset in self:
    ...
    def _get_disposal_moves(self):
        ...
        for asset in self:
            ...
            if unposted_depreciation_line_ids:
                ...
                tracked_fields = self.env['custom.account.asset.asset'].fields_get(['method_number', 'method_end'])
    ...
    def _entry_count(self):
        for asset in self:
            res = self.env['custom.account.asset.depreciation.line'].search_count([('asset_id', '=', asset.id), ('move_id', '!=', False)])
    ...
    def onchange_category_id_values(self, category_id):
        if category_id:
            category = self.env['custom.account.asset.category'].browse(category_id)
            return {
                'value': {
    ...
    def _compute_entries(self, date, group_entries=False):
        depreciation_ids = self.env['custom.account.asset.depreciation.line'].search([
...
class AccountAssetDepreciationLine(models.Model):
    _name = 'custom.account.asset.depreciation.line'
    _description = 'Asset depreciation line'

    name = fields.Char(string='Depreciation Name', required=True, index=True)
    sequence = fields.Integer(required=True)
    asset_id = fields.Many2one('custom.account.asset.asset', string='Asset',
