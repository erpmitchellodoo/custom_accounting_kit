...
class RecurringPayment(models.Model):
    _name = 'custom.recurring.payment'
    ...
    template_id = fields.Many2one('custom.account.recurring.template', 'Recurring Template',
    ...
    line_ids = fields.One2many('custom.recurring.payment.line', 'recurring_payment_id', string='Recurring Lines')
    ...
    def action_create_lines(self, date):
        ids = self.env['custom.recurring.payment.line']
        vals = {
    ...
    def action_generate_payment(self):
        line_ids = self.env['custom.recurring.payment.line'].search([('date', '<=', date.today()),
                                                                       ('state', '!=', 'done')])
        ...
        for vals in vals_list:
            if 'company_id' in vals:
                vals['name'] = self.env['ir.sequence'].with_context(force_company=vals['company_id']).next_by_code(
                    'custom.recurring.payment') or _('New')
            else:
                vals['name'] = self.env['ir.sequence'].next_by_code('custom.recurring.payment') or _('New')
...
class RecurringPaymentLine(models.Model):
    _name = 'custom.recurring.payment.line'
    _description = 'Recurring Payment Line'

    recurring_payment_id = fields.Many2one('custom.recurring.payment', string="Recurring Payment")
