...
class AccountBudgetPost(models.Model):
    _name = "custom.account.budget.post"
    _order = "name"
    _description = "Budgetary Position"

    name = fields.Char('Name', required=True)
    account_ids = fields.Many2many(
        'account.account', 'custom_account_budget_rel', 'budget_id',
        'account_id', 'Accounts'
    )
    company_id = fields.Many2one('res.company', 'Company', required=True, default=lambda self: self.env.company)

    def _check_account_ids(self, vals):
        # Raise an error to prevent the custom.account.budget.post to have not specified account_ids.
...
class CrossoveredBudget(models.Model):
    _name = "custom.crossovered.budget"
    _description = "Budget"
    ...
    crossovered_budget_line = fields.One2many(
        'custom.crossovered.budget.lines', 'crossovered_budget_id',
...
class CrossoveredBudgetLines(models.Model):
    _name = "custom.crossovered.budget.lines"
    _description = "Budget Line"

    name = fields.Char(compute='_compute_line_name')
    crossovered_budget_id = fields.Many2one('custom.crossovered.budget', 'Budget', ondelete='cascade', index=True, required=True)
    analytic_account_id = fields.Many2one('account.analytic.account', 'Analytic Account')
    analytic_plan_id = fields.Many2one(related='analytic_account_id.plan_id')
    general_budget_id = fields.Many2one('custom.account.budget.post', 'Budgetary Position')
    date_from = fields.Date('Start Date', required=True)
    date_to = fields.Date('End Date', required=True)
