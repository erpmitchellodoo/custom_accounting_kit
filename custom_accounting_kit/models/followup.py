...
class FollowupFollowup(models.Model):
    _name = 'custom.followup.followup'
    _description = 'Account Follow-up'
    _rec_name = 'name'

    name = fields.Char(string="Name", related='company_id.name', readonly=True)
    followup_line = fields.One2many('custom.followup.line', 'followup_id', 'Follow-up', copy=True)
...
class FollowupLine(models.Model):
    ...
    def default_get(self, default_fields):
        ...
        if self.env.ref('custom_accounting_kit.email_template_custom_account_followup_default'):
            values['email_template_id'] = self.env.ref('custom_accounting_kit.email_template_custom_account_followup_default').id
        return values
    ...
    followup_id = fields.Many2one('custom.followup.followup', 'Follow Ups',
