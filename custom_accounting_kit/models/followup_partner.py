...
class FollowupStatByPartner(models.Model):
    ...
    max_followup_id = fields.Many2one('custom.followup.line', 'Max Follow Up Level', readonly=True, ondelete="cascade")
    ...
    @api.model
    def init(self):
        tools.drop_view_if_exists(self.env.cr, 'custom_followup_stat_by_partner')
        self.env.cr.execute("""
            create view custom_followup_stat_by_partner as (
                SELECT
