...
class ResPartner(models.Model):
    ...
    def do_partner_print(self, wizard_partner_ids, data):
        ...
        datas = {
            ...
            'model': 'custom.followup.followup',
            'form': data
        }
        return self.env.ref(
            'custom_accounting_kit.action_report_followup').report_action(
            self, data=datas)

    def do_partner_mail(self):
        ctx = self.env.context.copy()
        ctx['followup'] = True
        template = 'custom_accounting_kit.email_template_custom_account_followup_default'
        unknown_mails = 0
        for partner in self:
    ...
    def get_followup_table_html(self):
        ...
        if partner.unreconciled_aml_ids:
            ...
            report = self.env['report.custom_accounting_kit.report_followup']
    ...
    def do_button_print(self):
        ...
        followup_ids = self.env['custom.followup.followup'].search(
            [('company_id', '=', company_id)])
        if not followup_ids:
    ...
    latest_followup_level_id = fields.Many2one(
        'custom.followup.line', compute='_get_latest', compute_sudo=True,
