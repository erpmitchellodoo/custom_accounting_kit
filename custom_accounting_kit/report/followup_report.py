...
class AccountFollowupStat(models.Model):
    ...
    followup_id = fields.Many2one('custom.followup.line', 'Follow Ups', readonly=True, ondelete="cascade")
    ...
    @api.model
    def init(self):
        tools.drop_view_if_exists(self.env.cr, 'custom_followup_stat')
        self.env.cr.execute("""
            create or replace view custom_followup_stat as (
                SELECT
                    l.id as id,
