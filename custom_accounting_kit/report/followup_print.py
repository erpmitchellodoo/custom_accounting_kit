...
class ReportFollowup(models.AbstractModel):
    _name = 'report.custom_accounting_kit.report_followup'
    _description = 'Report Followup'

    @api.model
    def _get_report_values(self, docids, data=None):
        model = self.env['custom.followup.sending.results']
    ...
    def _ids_to_objects(self, ids):
        all_lines = []
        for line in self.env['custom.followup.stat.by.partner'].browse(ids):
    ...
    def _get_text(self, stat_line, followup_id, context=None):
        fp_obj = self.env['custom.followup.followup']
