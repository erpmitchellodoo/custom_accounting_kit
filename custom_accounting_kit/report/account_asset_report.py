...
class AssetAssetReport(models.Model):
    _name = "custom.asset.asset.report"
    ...
    asset_id = fields.Many2one('custom.account.asset.asset', string='Asset', readonly=True)
    asset_category_id = fields.Many2one('custom.account.asset.category', string='Asset category', readonly=True)
    ...
    def init(self):
        tools.drop_view_if_exists(self.env.cr, 'custom_asset_asset_report')
        self.env.cr.execute("""
            create or replace view custom_asset_asset_report as (
                select
                    min(dl.id) as id,
                    dl.name as name,
                ...
                from custom_account_asset_depreciation_line dl
                    left join custom_account_asset_asset a on (dl.asset_id=a.id)
                    left join (select min(d.id) as id,ac.id as ac_id from custom_account_asset_depreciation_line as d inner join custom_account_asset_asset as ac ON (ac.id=d.asset_id) group by ac_id) as dlmin on dlmin.ac_id=a.id
